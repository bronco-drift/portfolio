...
export const projects: Project[] = [
  ...
  },
  {
    ...
    making: [
      {
        heading: { en: "Role", es: "Rol" },
        body: {
          en: "Product, design, and engineering end-to-end. Daily iteration with Claude over chat across three intense days from empty repo to production — a family PWA where each member posts one photo a day, live at albumfamiliar.vercel.app.",
          es: "Product, diseño y desarrollo end-to-end. Iteración diaria por chat con Claude durante tres días intensos desde el repo vacío hasta producción — PWA familiar donde cada miembro sube una foto por día, en vivo en albumfamiliar.vercel.app.",
        },
      },
      {
        heading: { en: "Key decisions", es: "Decisiones clave" },
        bullets: [
          {
            en: "Rejected an 'MVP without a backend' — with nine phones seeing each other's photos, throwing away localStorage would have been guaranteed rework. Designed the schema in the first pass with UNIQUE (room, member, day) and day_key stored in the room's timezone.",
            es: "Rechacé un 'MVP sin backend' — con nueve teléfonos viendo fotos ajenas, tirar localStorage era retrabajo garantizado. Diseñé el schema en la primera pasada con UNIQUE (sala, miembro, día) y day_key guardado en la timezone de la sala.",
          },
          {
            en: "Stable storage: a rolling 30-day window plus client-side WebP compression keeps the app at ~100 MB steady-state — comfortably inside Supabase's free tier for the long run.",
            es: "Storage estable: ventana rodante de 30 días + compresión WebP client-side dejan la app en ~100 MB constantes — cómodo dentro del free tier de Supabase a largo plazo.",
          },
          {
            en: "Google one-tap + magic-link fallback so aunts and uncles enter in two taps without inventing a password. Dev-mode env vars skip the login screen while keeping RLS exercised for real.",
            es: "Google one-tap + magic-link como fallback para que los tíos entren en dos taps sin inventar contraseña. Env vars de dev-mode saltean el login pero mantienen RLS ejercitado de verdad.",
          },
          {
            en: "Admin panel with storage and DB metrics plus a toggle to pause new registrations without changing the visual state of the landing.",
            es: "Panel admin con métricas de storage y base + toggle para pausar registros nuevos sin cambiar la estética visual de la landing.",
          },
          {
            en: "Built an in-app global chat as a direct channel to file bugs from the phone during real use — Claude reads and responds in the same channel via a subscription loop.",
            es: "Chat global interno como canal directo para reportar bugs desde el teléfono en pleno uso real — Claude lee y responde en el mismo canal vía un loop de suscripción.",
          },
        ],
      },
      {
        heading: { en: "Working with Claude", es: "Con Claude" },
        body: {
          en: "Owned scope discipline throughout ('hold on — the important thing is X'), cut gamification loops that didn't land, defined tight constraints (family across three continents, no user-count in the UI, immersive as the default), and ran QA on my real iPhone — Claude implemented, I tested on device and filed each issue in the same chat.",
          es: "Manejé la disciplina de scope todo el tiempo ('para un segundo, lo importante es X'), corté loops de gamification que no cerraban, definí constraints concretas (familia en tres continentes, no exponer cantidad de usuarios en UI, inmersivo como default), y llevé QA sobre mi iPhone real — Claude implementaba, yo probaba en dispositivo y reportaba cada issue en el mismo chat.",
        },
      },
    ],
    sections: [
